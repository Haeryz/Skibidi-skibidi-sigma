...
class PlanController extends GetxController {
  ...
  Future<void> scheduleNotificationForTrip(String tripId, String tripName, String destination, DateTime tripDate) async {
  ...
  await AwesomeNotifications().createNotification(
    content: NotificationContent(
      ...
      body: 'Your trip to $destination is today!',
      notificationLayout: NotificationLayout.BigText,
      category: NotificationCategory.Alarm, // Changed from REMINDER to alarm
    ),
    ...
    try {
      ...
      if (connectivityResult == ConnectivityResult.none) {
        ...
        if (isEdit && documentId != null) {
          ...
          DocumentReference docRef = await firestore.collection('trips').add(tripData);
          tripId = docRef.id;
        }

        // Schedule notification for the trip date
        DateTime tripDate = DateFormat('dd MMMM yyyy').parse(taskDate);
        await scheduleNotificationForTrip(tripId, name, destination, tripDate);
  ...
  Future<void> checkArrivalNotifications() async {
    final currentDate = DateTime.now();
    ...
    try {
      ...
      for (var doc in tripsSnapshot.docs) {
        ...
        final arrivalDateText = tripData['date'] as String;
        final tripDate = DateFormat('dd MMMM yyyy').parse(arrivalDateText);
        ...
        if (isSameDay(tripDate, currentDate)) {
          await AwesomeNotifications().createNotification(
            content: NotificationContent(
              id: doc.id.hashCode,
              channelKey: 'basic_channel',
              title: 'Trip Today: ${tripData['name']}',
              body: 'Your trip to ${tripData['destination']} is today!',
              notificationLayout: NotificationLayout.BigText,
